...
#ifndef _HDUDISPFBFCFONT_H_
#define _HDUDISPFBFCFONT_H_
...
/* Is Little Endian if set, or is Big Endian (default) */
#define LITTLE_ENDIAN1          (1<<8)
...
typedef struct T_BFC_FONT_PROP
{
   ...
   const struct T_BFC_FONT_PROP  *pNextProp; /* pointer to next T_BFC_FONT_PROP */
} T_BFC_FONT_PROP;
...
typedef struct
{
  ...
  union
   {
      const void        * pData;
      const T_BFC_FONT_MONO   * pMono; /* point to Monospaced font */
      const T_BFC_FONT_PROP   * pProp; /* point to proportional font */
   }p_var_type;
} T_BFC_FONT;
